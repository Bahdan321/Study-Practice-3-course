можешь пожалуйста создать новыый класс для работы с бд, но который будет основан на пакете sqlite, а не sqlalchemy, нужно будет также повторить все методы и функционал в них.Вот код:
