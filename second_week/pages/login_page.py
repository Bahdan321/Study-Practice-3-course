        ...
        if user:
            ...
            if remember_me:
                ...
                if token:
                    ...
                    print(
                        f"Persistent session created, token stored locally: {token[:8]}..."
                    )
                else:
            ...
            # Удаляем токен только если он есть
            if page.session.contains_key("session_token"):
                page.session.remove("session_token")
            # Также удаляем из client_storage, если он там есть
            if page.client_storage.contains_key("session_token"):
                page.client_storage.remove("session_token")
            page.update()
        ...
        [
            ft.Column(
                [
                    ...
                    error_text,
                    ft.TextButton(
                        "Нет аккаунта? Зарегистрироваться", on_click=go_to_register
                    ),
                ],
